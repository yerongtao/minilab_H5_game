...
var Main = (function (_super) {
  ...
  p.onPreloadRender = function () {
    ...
    this.beMaskShape.graphics.beginFill(0xf6a422);
  ...
  p.decodeMerge = function (json, texture) {
    if (json && texture) {
      ...
      for (var key in spriteSheetFrames) {
        ...
        ls.LayoutDecoder.spriteSheetDatas["resource/userAsset/" + key] = {
          offsetX: spriteObject.offX,
          offsetY: spriteObject.offY,
          texture: itemTexture
        };
      }
    }
  ...
  p.updateMask = function (itemsLoaded, itemsTotal, type) {
    // console.log("updateMask 打印");
    LoadingCount++;
    var DomProgress = document.getElementById("progress"),
      DomLoadingLife2 = document.getElementById("life2"),
      LoadingWidth = Math.floor(LoadingCount / 22 * 100);
    // console.log("这里看一下 LoadingWidth = " + LoadingWidth + " LoadingCount = " + LoadingCount);
    DomProgress.innerHTML = "LOADING " + LoadingWidth + " %";
    DomLoadingLife2.style.width = LoadingWidth + "%";
    if (LoadingWidth >= 95) {
      DomLoadingLife2.style.width = "95%";
    }
    if (LoadingWidth >= 100) {
      DomProgress.innerHTML = "LOADING 100 %";
    }
    if (this.beMaskShape) {
  ...
  p.onLimitResourceLoadComplete = function (event) {
    ...
    for (var i = 0; i < groups.length; i++) {
      ...
      ls.LayoutDecoder.spriteSheetDatas["resource/userAsset/" + resourceItem.name] = {
        offsetX: 0,
        offsetY: 0,
